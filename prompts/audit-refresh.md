# Refreshing wiki/synthesis/suggested-additions.md
...
Run `researchwiki audit --json`, diff against the existing page, insert into the right tier:
...
- Use `count_normalized` (= `multi_paper_count / total_papers`), not raw counts, to compare across audits.
