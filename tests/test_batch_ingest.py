...
def test_relay_watcher_surfaces_existing_and_new_prompts(
    wiki,
    monkeypatch,
    capsys,
):
    """Existing deterministic requests remain actionable after a CLI restart."""
    import threading
    import time

    relay = wiki / ".llm-relay" / "pending"
    relay.mkdir(parents=True)
    old = relay / "old.prompt.json"
    old.write_text('{"op_id":"old","phase":"author","pdf":"old.pdf"}')

    stop = threading.Event()
    thread = threading.Thread(
        target=_ingest_batch._watch_relay_prompts,
        args=(stop, set()),
    )
    thread.start()
    new = relay / "new.prompt.json"
    new.write_text('{"op_id":"new","phase":"judge","pdf":"new.pdf"}')
    deadline = time.monotonic() + 2
    observed = ""
    while not all(name in observed for name in ("new.pdf", "old.pdf")) and time.monotonic() < deadline:
        time.sleep(0.05)
        observed += capsys.readouterr().err
    stop.set()
    thread.join(timeout=1)
    observed += capsys.readouterr().err
    assert "new.pdf" in observed
    assert "old.pdf" in observed
