...
1. **No web search for prose content.** Never `WebSearch`/`WebFetch` to pull summaries or facts into the wiki. Every claim grounds in a PDF we have.
   ...
   **Exception — agent-native web scouting.** Only when the user explicitly asks for `researchwiki scout web`, the active chat agent may use its own web-search harness for broad discovery. The CLI emits a bounded handoff but has no web-search provider of its own; the agent gives its normal conversational answer with native citations, then records only a minimal, self-attested source receipt (harness, `discovery_method`, URL, whether the page was opened, and optional title/date). `discovery_method` is required and says how the URLs were found — `search`, `fetch-only` (fetch worked, no search tool, so URLs came from model priors), or `user-provided-url`; the two searchless modes reject search-only sources, since nothing searched. The CLI rejects research prose and caches the receipt as `discovery-only` under `.scout-cache/`; `show <run-id>` displays that same cached result on demand and creates no formal report. A `--since` date rejects known older publication dates but accepts undated sources as date-unverified. The receipt cannot verify what the host harness actually searched or fetched — `discovery_method` narrows that to a declared mode but is still self-attested. A run whose agent work never happened stays `requested`; never close it with a zero-source receipt, which means *searched and found nothing*. Never copy or paraphrase the answer or cached result into wiki prose, claims, or wikilinks—obtain and ingest the underlying PDF first. On a resumed session run `researchwiki scout web list` / `show <run-id> --json` rather than duplicating a request. Read [`prompts/scout-web.md`](./prompts/scout-web.md) before running it.
