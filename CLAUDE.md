...
**Corollary — cross-links must be source-supported.** `[[wikilink]]` only when the source explicitly cites/builds on/contrasts the other paper. Topical adjacency alone is not enough. Semantic similarity may nominate a candidate, but cannot verify it; the link-generation judge must find explicit build-on/contrast evidence in the supplied source-grounded excerpts, and stub/borderline verdicts produce no link.
...
**Corollary — offer proposals the user may not know exist.** After a substantive cross-paper answer that surfaces a tension, a shared mechanism, or a method one category could lend another, close with one line offering a proposal on it, and include the exact `researchwiki proposals generate "<question>" --papers <stems>` command. Offer; never run it unasked (it spends a model call). `researchwiki proposals opportunities --json` lists what the corpus already signals; it's local and free, so check it when the question touches a paper it names. Skip for single-paper or factual lookups.
...
└── wiki/                   # Wiki pages (English)
    ...
    ├── proposals/          # Synced proposal + feedback decision ledger
    ├── concepts/           # Legacy single-term hubs (optional)
...
- **Scaffold**: `synthesis`, `ideas`, `proposals`, `references`, `other` — created by `researchwiki init` (`categories.DEFAULT_DIRS`), not committed. First four are **page-type dirs**, not content categories; `other` is the classifier's abstention bucket. Existing `concepts/` remains a recognized legacy page-type directory but is not created for new wikis.
...
author_model: "gpt-5.6-luna"              # exact model id that authored the page; replace with the active model
...
author_model: "gpt-5.6-luna"          # required: exact model id that authored the page; quote it
...
**Manual workflow**: reference docs skip `researchwiki ingest`. Move PDF into `papers/`, write the page directly, set `author_model:` to the exact model id used to author the page (not `TODO` or a generic label), stamp `ingested_at`, update `index.md`, append to `log.md`. `lint` requires this field on every content document type: paper, commentary, synthesis, concept, idea, and reference documents. Only mechanically maintained `meta` / `dashboard` pages are exempt. For legacy pages whose model was never recorded, never invent one: use the reviewed `researchwiki migrate provenance` path below.
...
**Manual workflow** — see [`prompts/idea-page-author.md`](./prompts/idea-page-author.md) (section-by-section guidance, verdict-label criteria, verify steps). Skip `researchwiki ingest`; set `author_model:` to the exact model id that authored the current page, move drafts into `wiki/ideas/`, update `index.md`, append to `log.md`.
...
### 5. Proposal record — `wiki/proposals/{slug}--{id}.md`

A synced, reviewable candidate for a future synthesis or idea page. Generate with `researchwiki proposals generate "<question>"`; preview is the default and `--write` persists at most three. The generator starts from the claims DB, keeps the evidence packet paper-diverse and bounded, and explores only five directions: **tension**, **shared mechanism**, **complementary limitations**, **boundary condition**, and **cross-category application**. Cross-category mode is target-problem-first: pass `--target-category <category> --cross-category`; one planning call translates the target problem into capabilities, then claim retrieval searches outside the target category. A chat agent can supply that plan itself with `--search-plan PLAN.json`, which skips the planner call (and, with `--prepare-only`, yields a packet carrying source evidence). Generation preflights the provider and author model like `agent ingest`, and a receipt must name an exact author model.

Markdown is canonical. YAML records `proposal_id`, proposed page type, direction, status, source fingerprint, optional parent/resulting page, model, and timestamps. Body evidence uses durable `[[stem#claim_slug]]` citations selected only from the retrieved packet. `## Feedback` is an append-only readable decision history; record it with `researchwiki proposals feedback <id> --decision <status> --reason "…"`. Statuses are `proposed`, `shortlisted`, `deferred`, `rejected`, `drafted`, and `published`. `state.db` tables mirror these pages only for filtering and are fully recreated by `researchwiki db rebuild`.

Proposal records are not citable evidence and do not need a catalog `hook:`. Once accepted, draft the resulting page under `synthesis/` or `ideas/` and apply that page type's grounding gates. Full procedure: [`prompts/proposal-workflow.md`](./prompts/proposal-workflow.md).

### Deprecated concept page — `wiki/concepts/{slug}.md`

Deprecated; removed no earlier than 0.7.0 and 2026-12-24 (`concept-hubs` in `researchwiki/data/deprecations.yaml`). Until then existing single-term hubs stay readable and the `concepts` / `candidates concepts` commands work, printing a notice on stderr. New wikis do not scaffold them, ingest does not attach papers, and `status` does not promote candidates. Use proposal records for discovery. To retire hubs early, follow [`prompts/proposal-workflow.md`](./prompts/proposal-workflow.md) § Deprecated concept hubs; hub maintenance is in [`prompts/concept-page-author.md`](./prompts/concept-page-author.md).
...
- `views.md` — Dataview dashboard (`type: dashboard`), scaffolded by `researchwiki init`. Renders only inside Obsidian. Its proposal table shows intended page type, direction, status, and creation date; a dashboard scaffolded before proposals may keep its concept-hub table in that slot instead, and lint checks whichever is present. Synthesis intentionally has no Members column or DataviewJS dependency. `researchwiki lint` occasionally checks these semantic layout rules under `dashboard_contract_violations` without requiring byte-for-byte template identity. Root bookkeeping pages carry no `category:` — the parent dir is `wiki`, so adding one trips category-drift.
...
- Every page declares `type:` — `paper`, `synthesis`, `guidance`, `protocol`, `whitepaper`, `book`, `idea`, `proposal`, legacy `concept`, `commentary`, plus `meta` / `dashboard` for the wiki-root bookkeeping pages. Proposal/meta/dashboard pages are exempt from `hook:`; root meta/dashboard pages are skipped by `db rebuild`.
- Every content document declares `author_model:` with the exact model id that authored its current prose. This covers paper, commentary, synthesis, concept, idea, proposal, and reference-document types; mechanically maintained `meta` / `dashboard` pages are exempt.
...
- Proposal records follow the Markdown contract emitted by `researchwiki proposals`; they are decision artifacts, not citable evidence.
- Deprecated concept pages must follow Definition → How it appears across the corpus (Cross-domain connections optional).
...
**Default path: `researchwiki agent ingest`.** Handles pypdfium2 extraction, DOI detection, S2 lookup, LLM-reconcile, stem derivation, `mv` to `papers/`, page authoring, and atomic back-link apply. Synthesis evolution is opt-in with `--memory-evolve` or on demand via `researchwiki evolve`. Logs `ingest_iterations` to the state DB; writes no `tags:` (see Page Types).
...
**Step 1** — run agent ingest. **For ≥2 PDFs pass them all to a single `agent ingest` invocation** — it auto-enters crash-safe batch mode (4 workers on an API provider or `--stub`, 1 under chat-relay, atomic `.ingest/batch-<ts>/checkpoint.json` per completion, `--resume <batch-dir>` picks up after a crash; resume re-resolves the provider default but keeps an explicit `-w N`). Do NOT fan out one background Bash per file: it bypasses the checkpoint, uncaps concurrency, and multiplies `state.db` write contention. `agent ingest` takes **no `--category`** — the classifier picks one per paper and abstains to `other`. (`--category` is the digest path's flag, on `researchwiki ingest`.)

**Run it backgrounded — API providers only.** One invocation, backgrounded, whether it's 1 PDF or 20, so the conversation continues; this is not the banned per-file fan-out, and it needs no subagent (a subagent spends a whole context waiting on stdout you have to read yourself anyway). **Under chat-relay, run it in the foreground**: each worker forwards its relay handoffs to the batch parent's stderr, and backgrounding hides exactly the prompt you have to answer — every phase then dies on its timeout. See the chat-relay exception below. While it runs the wiki is **read-only**: no `reindex` / `db rebuild` / `lint` / `grade` until the completion notification lands, or they contend with promote's writes to `state.db`, `wiki/`, `index.md` and `log.md` — and a `reindex` that wins the race indexes a half-landed tree.
...
researchwiki agent ingest inbox/*.pdf                           # ≥2 PDFs — batch + checkpoint; API=4 workers, chat-relay=1
...
Optional per-PDF guardrails are `--max-model-calls`, `--max-tokens`, `--max-cost-usd`, and `--max-wall-seconds`; batch mode forwards them to every worker. A limit is reserved before a model call, so parallel drafts cannot oversubscribe it. Exhaustion writes a terminal `budget-exhausted` event and preserves the best graded partial under `.agent-output/` when one exists. Cost budgets refuse an unpriced cloud model instead of pretending it is free. Required grade persistence happens immediately after promotion, with enforcement paused so a wall deadline cannot interrupt local maintenance on an already-landed page. Optional model-backed post-hooks (`--memory-evolve`, `--claim-overlap`, `--contradiction-alert`) re-arm the same tracker and record a skip rather than failing an already-promoted ingest when the budget is exhausted.
...
**Never write ad-hoc scripts to ingest PDFs, and never fan out one Bash task per file.** Always use `agent ingest` (or digest path for recovery); rely on its built-in batch mode for multi-PDF runs. **One exception: `chat-relay`** — when you invoke a multi-PDF ingest and native subagents are available, actively supervise a bounded rolling pool with one foreground single-PDF invocation per subagent. Each subagent owns one paper through its paper-specific post-ingest work; the main agent monitors terminal status, retries only transient failures, and runs corpus-wide post-batch work once. Without subagents, use the visible sequential batch (chat-relay defaults to one worker); explicit `-w N` opts into concurrent requests but does not create isolated chat contexts. See [`prompts/chat-relay.md`](./prompts/chat-relay.md#parallel-ingests--supervised-fan-out).

**Metadata adoption gate.** A DOI is adopted only if the resolved record *is this paper* (`metadata_sanity`: first-author surname present, year within ±1, ≥50% title-content-word overlap) — resolvability alone is not enough, and the URL-DOI hunt no longer reads past the References heading, where every DOI belongs to somebody else. When the PDF prints no usable DOI, an S2 title match is tried under the same gate. A masthead-derived venue naming typesetting furniture is dropped rather than recorded; `lint`'s `venue_suspect` and `backfill doi --verify` are the backstops. Verified cross-links are written in **both** directions at promote time. Citation-graph evidence verifies directly; semantic KNN only nominates, and its judge keeps a candidate only when the source-grounded excerpts explicitly build on or contrast it. The retained legacy kind is `topical`, and promotion upgrades it to a citation kind when the source PDF's reference list proves one. Thin extraction (no results/discussion section, or zero PDF-side claims) logs a warning — the page can still pass every gate on a thinner grounding corpus than it appears to have.

**Coverage gate.** Target-claim extraction is required for a new ingest: unavailable, errored, or empty extraction blocks automatic promotion, as does any missed claim marked `critical`. A single critical target miss triggers the revision critic; the older two-gap threshold remains only for non-target structural anchors. The target-claim pass also proposes the page's 5–10 source-derived keywords, so the normal path does not spend a separate keyword call; malformed or insufficient keywords fall back to the dedicated keyword phase. Long but healthy PDFs use a stratified document sample rather than only the opening prefix. Limitations must be explicitly stated or directly demonstrated by the PDF — the extractor must not invent a reviewer-style gap from absence.

**Terminal outcomes are distinct.** `promoted` means the page and canonical PDF landed; `already_present` means the deposited bytes exactly match an existing canonical PDF and no downstream authoring, grading, or hooks run (the input is left unchanged); `sandboxed` means a review draft landed only in `.agent-output/`. An automatic gate failure exits **1**, so batch checkpoints cannot label it successful; `--force-sandbox` remains an intentional exit-0 review run. Only `promoted` runs post-promote hooks. Supplementary files passed with `--supplementary` are attached and declared, but are not analyzed as evidence during ingest.
...
**Step 2 — verify index health.** Successful agent promotion upserts the new page and every changed back-link target into both Tantivy and the semantic index under a cross-process lock. A missing index is built automatically. Run `researchwiki reindex` only after a reported incremental-update failure, after a manual/digest-path page write, or when `status` reports stale/missing indexes; it remains the full recovery builder.
...
**Step 2.6 — Proposal discovery is user-initiated, but surfaced.** Ingest does not mutate cross-paper hubs or spend a proposal-generation call. When the new paper joins a proposal opportunity (a judged contradiction or build-on/refine edge), ingest prints one `Proposal opportunity:` line with the exact command; this is a single local read and costs nothing. When a paper or question exposes a worthwhile gap, preview `researchwiki proposals generate "<question>" --papers <stem>`; add `--target-category <category> --cross-category` to search for transferable methods elsewhere. Review the full saved receipt, then use `proposals accept <file> --select 1` to save the exact choice without regeneration. `--prepare-only` makes no model calls. Saved Markdown and feedback sync with the wiki and later proposal runs receive the relevant decision history. See [`prompts/proposal-workflow.md`](./prompts/proposal-workflow.md).
...
`researchwiki migrate provenance` is the beta upgrade path for pages flagged by `missing_author_model`. The default is plan-only: it writes a hash-bound review manifest under `.ingest/`, preselecting only exact models recoverable from ingest telemetry. Review every remaining page as `set-author-model`, `acknowledge`, or `skip`, then apply with `researchwiki migrate provenance --apply --run <dir>`. Apply refuses pending/stale/conflicting entries, creates `pre-apply.tar.gz`, and is idempotent. `acknowledge` writes `author_provenance: legacy-unrecorded` plus `provenance_acknowledged_at: YYYY-MM-DD`; `lint` reports these separately rather than repeating them as missing. Never infer a model from prose or timestamps.
...
- **`researchwiki status`** — instant, local dashboard: paper count per category, cross-link graph, `inbox/` backlog, resumable web-scout handoffs, orphaned PDFs in `papers/`, recent additions, saved proposals awaiting feedback, and proposal opportunities latent in the corpus (`Proposal opportunities: …` → `researchwiki proposals opportunities`). Run after every ingest session, and at minimum every ~5 ingests since the last run. Several signals surface only in aggregate: index staleness, structured-DB drift, cumulative orphans, cross-session backlog, and the 7-day cost rollup. `Claim-overlap backlog: N stem(s) pending` triggers `researchwiki claim-overlap --backlog`; `Claim-pair discovery: N unreviewed cross-category pair(s)` triggers `researchwiki candidates pairs --cross-category`.
...
- **`researchwiki proposals opportunities [--json]`** — ranked proposal opportunities the corpus already contains, each with a ready `proposals generate` command. Local and free: judged claim-graph contradictions (first), then build-on/refine chains grouped into one entry per connected set, paper clusters of 3–40 with no synthesis page, and cross-category claim pairs sharing a named method. Topics an existing proposal already cites are dropped. `status` shows a decay-stamped summary of the judged-edge signals only (14-day quiet period); the cluster and pair scans take seconds, so they run only in this command.
- **`researchwiki proposals generate "<question>" [--papers STEM ...] [--write]`** — primary creative page-discovery path. It retrieves a bounded, paper-diverse claim packet, includes relevant prior feedback, and asks for at most three discriminating synthesis/idea proposals across tension, shared-mechanism, complementary-limitations, boundary-condition, and cross-category-application directions. Preview is default. For target-problem-first transfer, add `--target-category CAT --cross-category`. List with `proposals list`; remember decisions with `proposals feedback`. Full procedure: [`prompts/proposal-workflow.md`](./prompts/proposal-workflow.md).
- **`researchwiki candidates concepts [--bridges] [--json]`** — deprecated with concept hubs (see Page Types); still works and warns on stderr until removal. It can still mine recurring contribution-claim vocabulary and scaffold a hub, but new wikis do not create the directory, ingest never auto-attaches papers, and `status` does not surface this queue.
...
| **1** | User-input error / no-result-where-expected / review-required ingest gate outcome. |
