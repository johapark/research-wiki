...
**Manual workflow**: reference docs skip `researchwiki ingest`. Move PDF into `papers/`, write the page directly, set `author_model:` to the exact model id used to author the page (not `TODO` or a generic label), stamp `ingested_at`, update `index.md`, append to `log.md`. `lint` checks this field. Evolving `idea` pages are the intentional exception and do not carry `author_model:`; synthesis and concept pages do. For legacy pages whose model was never recorded, never invent one: use the reviewed `researchwiki migrate provenance` path below.
