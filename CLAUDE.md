...
Existing single-term hubs remain supported for backward compatibility. They are strict-grounded mini-syntheses with `referenced_papers:` spokes, but new wikis do not scaffold them, ingest does not attach papers automatically, and `status` does not promote concept candidates. Prefer proposal records for new discovery because they preserve a question, thesis, competing evidence, decision, and feedback rather than only recurring vocabulary. Manual maintenance remains available through `researchwiki concepts`; see [`prompts/concept-page-author.md`](./prompts/concept-page-author.md).
...
- `views.md` — Dataview dashboard (`type: dashboard`), scaffolded by `researchwiki init`. Renders only inside Obsidian. Its proposal table shows intended page type, direction, status, and creation date; synthesis intentionally has no Members column or DataviewJS dependency. `researchwiki lint` occasionally checks these semantic layout rules under `dashboard_contract_violations` without requiring byte-for-byte template identity. Root bookkeeping pages carry no `category:` — the parent dir is `wiki`, so adding one trips category-drift.
