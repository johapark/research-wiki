...
Optional per-PDF guardrails are `--max-model-calls`, `--max-tokens`, `--max-cost-usd`, and `--max-wall-seconds`; batch mode forwards them to every worker. A limit is reserved before a model call, so parallel drafts cannot oversubscribe it. Exhaustion writes a terminal `budget-exhausted` event and preserves the best graded partial under `.agent-output/` when one exists. Cost budgets refuse an unpriced cloud model instead of pretending it is free. Successful promotion pauses enforcement so a wall deadline cannot interrupt required local maintenance on an already-landed page. The optional memory-evolution phase briefly re-arms the same tracker, so it still spends this ingest's budget; exhaustion there is a recorded skip rather than a failed ingest. Enforcement stays paused for grade persistence. Everything between promote and memory evolution is local-only by invariant — never add a metered call there.
