...
Optional per-PDF guardrails are `--max-model-calls`, `--max-tokens`, `--max-cost-usd`, and `--max-wall-seconds`; batch mode forwards them to every worker. A limit is reserved before a model call, so parallel drafts cannot oversubscribe it. Exhaustion writes a terminal `budget-exhausted` event and preserves the best graded partial under `.agent-output/` when one exists. Cost budgets refuse an unpriced cloud model instead of pretending it is free. Promotion does not by itself release the guard: the optional memory-evolution call still spends this ingest's budget, and exhaustion there is a recorded skip rather than a failed ingest. The guard is released once that call has run or skipped, so grade persistence finishes rather than leaving a half-maintained page. Everything between promote and that point is local-only by invariant — never add a metered call there.
