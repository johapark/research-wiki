...
1. **No web search for prose content.** Never `WebSearch`/`WebFetch` to pull summaries or facts into the wiki. Every claim grounds in a PDF we have.
   ...
   **Exception — agent-native web scouting.** Only when the user explicitly asks for `researchwiki scout web`, the active chat agent may use its own web-search harness for broad discovery. The CLI emits a bounded handoff but has no web-search provider of its own; the agent gives its normal conversational answer with native citations, then records only a minimal, self-attested source receipt (harness, URL, whether the page was opened, and optional title/date). The CLI rejects research prose and stores the receipt as `discovery-only` under `.scout-cache/`; `output/scout/` is only an optional source ledger. A `--since` date rejects known older publication dates but accepts undated sources as date-unverified. The receipt cannot verify what the host harness actually searched or fetched. Never copy or paraphrase the answer or ledger into wiki prose, claims, or wikilinks—obtain and ingest the underlying PDF first. On a resumed session run `researchwiki scout web list` / `show <run-id> --json` rather than duplicating a request. Read [`prompts/scout-web.md`](./prompts/scout-web.md) before running it.
...
├── .scout-cache/           # Quarantined agent-native web leads
├── output/scout/           # Discovery-only rendered reports
...
- `scout web list --json` → `{schema_version, runs[]}`; each run carries `{run_id, state, query, created_at, source_count, next_command, error}`. States are `requested`, `recorded`, `invalid`; the optional report has no separate state. `scout web show <run-id> --json` re-emits the versioned request plus `request_path` so a different chat-agent host can resume it exactly.
- `scout web report <run-id> --json` → `{schema_version, run_id, evidence_class, source_count, fetched_count, report_path}`. The report is a source ledger; the host agent's cited conversational answer remains the research deliverable.
