...
### Legacy concept page — `wiki/concepts/{slug}.md`

Existing single-term hubs remain supported for backward compatibility. They are strict-grounded mini-syntheses with `referenced_papers:` spokes, but new wikis do not scaffold them, ingest does not attach papers automatically (`researchwiki concepts attach <stem> ...` does it on request), and `status` does not promote concept candidates. Prefer proposal records for new discovery because they preserve a question, thesis, competing evidence, decision, and feedback rather than only recurring vocabulary. Manual maintenance remains available through `researchwiki concepts`; see [`prompts/concept-page-author.md`](./prompts/concept-page-author.md).
...
- Legacy concept pages must follow Definition → How it appears across the corpus (Cross-domain connections optional).
...
- **`researchwiki candidates concepts [--bridges] [--json]`** — legacy manual support for existing concept-hub users. It can still mine recurring contribution-claim vocabulary and scaffold a hub, but new wikis do not create the directory, ingest never auto-attaches papers, and `status` does not surface this queue.
