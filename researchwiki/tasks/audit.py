"""Citation-graph audit across all current wiki papers.
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        ...
        description="Audit the wiki's citation graph against Semantic Scholar.",
    )
    ...
    # audited (no DOI → no S2 lookup). Lint catches them via its
    # `missing_doi` check; audit just acknowledges the skip count so
    # the denominator (papers_audited / total_paper_pages) is honest.
    # Pages that declare `no_doi_reason:` are intentional gaps (NeurIPS
    # posters, workshop papers, etc.) — audit excludes them from the
    ...
    if skipped_no_doi > 0:
        log(f"WARN: {skipped_no_doi} paper page(s) skipped (no DOI in YAML); "
            f"audit denominator is {len(papers)}/{eligible}. "
            f"Run `researchwiki lint --json | jq .missing_doi` to fix.",
            tag="audit")
    if intentional_no_doi:
        log(f"Note: {len(intentional_no_doi)} page(s) excluded from audit by "
            f"`no_doi_reason:` field (legitimate no-DOI cases).",
            tag="audit")
    log(f"Wiki papers (auditable): {len(papers)}", tag="audit")
    doi_to_stem = {(p["doi"] or "").lower(): p["stem"] for p in papers if p.get("doi")}
    stem_to_category = {p["stem"]: p["category"] for p in papers}
    total_papers = len(papers)
    provider = get_default_provider(
        log_tag="audit", force_refresh_days=args.refresh_cache,
    )

    # Prefetch all metadata in one batch; get_by_doi calls below hit cache.
    dois_to_batch = [p["doi"] for p in papers if p.get("doi")]
    if dois_to_batch:
        log(f"batch-fetching metadata for {len(dois_to_batch)} papers", tag="audit")
        provider.get_batch_metadata(dois_to_batch)
    ...
    for p in papers:
        stem = p["stem"]
        log(f"Paper {stem}...", tag="audit")
        article = provider.get_by_doi(p["doi"]) if p.get("doi") else None
        if article is None:
    ...
    def _entry(doi: str, info: dict, *, sources_key: str) -> dict:
        ...
        return {
            ...
            "count_normalized": round(info["count"] / max(total_papers, 1), 4),
            "categories": cats,
    ...
    print("# Semantic Scholar Audit Report")
    print()
