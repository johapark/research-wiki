...
Usage:
  ...
  researchwiki agent ingest inbox/*.pdf             # batch + checkpoint; API=4 workers, chat-relay=1
...
def _indexed_claim_count(stem: str | None) -> int | None:
    ...
    try:
        ...
        try:
            row = conn.execute(
                "SELECT COUNT(*) FROM claims WHERE paper_stem = ?",
                (stem,),
            ).fetchone()
...
def _cmd_ingest(args) -> int:
    ...
    if not args.pdfs:
        print(f"{_prog()}: need PDF path(s) (or --resume BATCH_DIR)", file=sys.stderr)
        return 1
    ...
    for attr, flag in (
        ...
        if value is not None and value <= 0:
            print(f"{_prog()}: {flag} must be greater than zero", file=sys.stderr)
            return 1
    ...
    if batch_mode:
        ...
        conflicts = [
            flag
            for attr, flag in _BATCH_INCOMPATIBLE_FLAGS
            if getattr(args, attr, None)
        ]
        if conflicts:
            print(
                f"{_prog()}: cannot combine batch mode "
                f"({len(args.pdfs)} PDFs / --workers set) with per-PDF flags: "
                f"{', '.join(conflicts)}",
                file=sys.stderr,
            )
            print(
                "  fix: run these PDFs one at a time, or drop the flags.",
                file=sys.stderr,
            )
            return 1
        if args.auto_promote and args.force_sandbox:
            print(
                f"{_prog()}: --auto-promote and --force-sandbox are mutually exclusive",
                file=sys.stderr,
            )
            return 1
    ...
    try:
        ...
        print(
            "  no page was auto-promoted; raise the limit or resume with a fresh ingest",
            file=sys.stderr,
        )
        return 1
        ...
        print(
            f"  missing fields: {', '.join(e.missing) or '(unknown)'}", file=sys.stderr
        )
        ...
        if "doi" in e.missing:
            ...
            print(
                "  fix: re-run with `--year YYYY` (and `--title` if needed).",
                file=sys.stderr,
            )
        else:
            print(
        ...
        print(
            f"{_prog()}: promote did not complete — the paper is PARTIALLY landed.",
            file=sys.stderr,
        )
...
def _cmd_trace(args) -> int:
    ...
    print(
        f"  {'#':<3} {'role':<10} {'sect':<5} {'model':<22} {'in':>5} {'out':>5} "
    ...
    for r in rows:
        ...
        print(
            f"  {r.iteration:<3} {r.role:<10} {sec:<5} {model:<22} "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_ingest = subs.add_parser(
        "ingest", prog=_prog(), help="Run the ingest agent on one or more PDFs"
    )
    ...
    p_trace = subs.add_parser(
        "trace", help="Print every iteration row for an attempt_id"
    )
    p_trace.add_argument("attempt_id")
    p_trace.add_argument(
        "--show-drafts", action="store_true", help="Also print draft text"
    )
