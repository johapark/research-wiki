...
- **`prompts/*.md` + their CLAUDE.md pointers** — a trigger-gated procedure is
  ...
  and names the misses. `--dry-run` lists what it would test and spends nothing;
  it also reports prompt files no pointer reaches at all. Costs tokens, so it's
  on-demand — not part of `pytest`.
